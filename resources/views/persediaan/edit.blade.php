                    ...
                    <form action="{{ route('persediaan.update', $persediaan->id_barang) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('jenis_barang') is-invalid @enderror" id="jenis_barang" name="jenis_barang" required>
                                ...
                                <option value="barang_dagangan" {{ old('jenis_barang', $persediaan->jenis_barang) == 'barang_dagangan' ? 'selected' : '' }}>Persediaan Barang Dagangan</option>
                                
                                @if(in_array($jenisUsaha ?? 'dagang', ['pertanian', 'multi']))
                                <option value="aset_biologis" {{ old('jenis_barang', $persediaan->jenis_barang) == 'aset_biologis' ? 'selected' : '' }}>Persediaan Aset Biologis (PSAK 69)</option>
                                @endif
                                
                                @if(in_array($jenisUsaha ?? 'dagang', ['manufaktur', 'multi']))
                                <option value="bahan_baku" {{ old('jenis_barang', $persediaan->jenis_barang) == 'bahan_baku' ? 'selected' : '' }}>Persediaan Bahan Baku</option>
                                <option value="barang_dalam_proses" {{ old('jenis_barang', $persediaan->jenis_barang) == 'barang_dalam_proses' ? 'selected' : '' }}>Barang dalam Proses (WIP)</option>
                                <option value="barang_jadi" {{ old('jenis_barang', $persediaan->jenis_barang) == 'barang_jadi' ? 'selected' : '' }}>Barang Jadi</option>
                                @endif
                            </select>
