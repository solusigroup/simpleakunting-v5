...
<style>
    ...
    .searchable-select-trigger { display: flex; align-items: center; justify-content: space-between; width: 100%; padding: 0.5rem 1rem; font-size: 0.875rem; border: 1px solid var(--color-border, #dee2e6); border-radius: 8px; background: var(--color-bg, #fff); color: var(--color-text, #333); cursor: pointer; transition: all 0.2s ease; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-height: 40px; }
    .searchable-select-trigger:hover { border-color: var(--color-primary, #8b5cf6); }
    .searchable-select-trigger.open { border-color: var(--color-primary, #8b5cf6); box-shadow: 0 0 0 4px rgba(139, 92, 246, 0.15); z-index: 1051; }
    ...
    .searchable-select-trigger .trigger-chevron { margin-left: 8px; transition: transform 0.2s; flex-shrink: 0; color: #666; }
    .searchable-select-trigger.open .trigger-chevron { transform: rotate(180deg); color: var(--color-primary, #8b5cf6); }
    .searchable-select-dropdown { display: none; position: absolute; top: calc(100% + 6px); left: 0; right: 0; background: #fff; border: 1px solid var(--color-border, #dee2e6); border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.15); z-index: 1050; max-height: 320px; overflow: hidden; }
    .searchable-select-dropdown.show { display: block; }
    .searchable-select-search { padding: 12px; border-bottom: 1px solid #eee; position: sticky; top: 0; background: #f8f9fa; z-index: 1; }
    .searchable-select-search input { width: 100%; padding: 10px 14px; border: 1px solid #e0e0e0; border-radius: 8px; font-size: 0.875rem; outline: none; transition: all 0.2s; background: #fff; color: var(--color-text, #333); }
    .searchable-select-search input:focus { border-color: var(--color-primary, #8b5cf6); box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.1); }
    .searchable-select-options { max-height: 240px; overflow-y: auto; padding: 6px 0; }
    .searchable-select-option { padding: 10px 16px; cursor: pointer; font-size: 0.875rem; color: var(--color-text, #333); transition: background 0.15s; }
    .searchable-select-option:hover { background: #f5f3ff; color: var(--color-primary, #8b5cf6); }
                    ...
                    <div class="searchable-select" id="ss_${currentRow}">
                        ...
                        <div class="searchable-select-trigger" id="ss_trigger_${currentRow}">
                            <div class="trigger-text placeholder">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="opacity: 0.8;"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                                <span>Cari akun...</span>
                            </div>
    ...
    function initSearchableSelect(rowId) {
        ...
        optionsContainer.addEventListener('click', function(e) {
            ...
            triggerText.innerHTML = `
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
