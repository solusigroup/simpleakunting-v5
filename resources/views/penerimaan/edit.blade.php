...
@section('title', 'Edit Penerimaan Kas - Simple Akunting')

@section('content')
    <div class="page-header-actions">
        <div>
            <h1 class="page-title">Edit Penerimaan Kas</h1>
            <p class="page-subtitle">Ubah transaksi penerimaan kas atau bank</p>
        </div>
        <div>
    ...
    <form action="{{ route('penerimaan.update', $jurnal->id_jurnal) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="form-card-body">
                ...
                <div class="form-row mt-3">
                        ...
                        <select class="form-select" id="id_pelanggan" name="id_pelanggan" onchange="updateSaldoDisplay()">
                            ...
                            @foreach($pelanggan as $p)
                                <option value="{{ $p->id_pelanggan }}" 
                                    ...
                                    {{ old('id_pelanggan', $jurnal->id_pelanggan) == $p->id_pelanggan ? 'selected' : '' }}>
                                    {{ $p->nama_pelanggan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group" style="flex: 2;">
                        ...
                        <input type="text" class="form-control" id="keterangan" name="keterangan" value="{{ old('keterangan', $jurnal->deskripsi) }}" placeholder="Contoh: Penerimaan Setoran Modal" required>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    if (document.getElementById('id_cabang').value) {
        ...
        const existingDetails = @json($detailsKredit);
            ...
            existingDetails.forEach(detail => {
                ...
                let html = `
                    <tr id="row_${currentRow}">
                        ...
                        <td>
                            <input type="number" class="form-control form-control-sm input-jumlah" name="details[${currentRow}][jumlah]" value="${detail.kredit}" min="0" onkeyup="hitungTotal()" onchange="hitungTotal()">
                        </td>
                        <td>
                            <button type="button" class="btn btn-danger btn-icon btn-sm" onclick="hapusBaris(${currentRow})">
                                <span data-feather="x" style="width: 14px; height: 14px;"></span>
