...
@section('title', 'Edit Faktur Pembelian - Simple Akunting')

@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Faktur Pembelian: {{ $pembelian->no_faktur_pembelian }}</h1>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="no_faktur" value="{{ $pembelian->no_faktur_pembelian }}" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Tanggal</label>
                            <input type="date" class="form-control" name="tanggal_faktur" value="{{ date('Y-m-d', strtotime($pembelian->tanggal_faktur)) }}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="id_pemasok" required>
                                ...
                                @foreach($pemasok as $p)
                                    <option value="{{ $p->id_pemasok }}" {{ $pembelian->id_pemasok == $p->id_pemasok ? 'selected' : '' }}>{{ $p->nama_pemasok }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_cabang" name="id_cabang" required>
                                ...
                                @foreach($cabang as $c)
                                    <option value="{{ $c->id }}" {{ $pembelian->id_cabang == $c->id ? 'selected' : '' }}>{{ $c->kode_cabang }} - {{ $c->nama_cabang }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_unit_usaha" name="id_unit_usaha" required>
                                ...
                                @foreach($unitUsaha as $u)
                                    <option value="{{ $u->id }}" data-cabang="{{ $u->id_cabang }}" {{ $pembelian->id_unit_usaha == $u->id ? 'selected' : '' }}>{{ $u->kode_unit }} - {{ $u->nama_unit }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_project" name="id_project">
                                ...
                                @foreach($projects as $proj)
                                    <option value="{{ $proj->id_project }}" data-unit="{{ $proj->id_unit_usaha }}" {{ $pembelian->id_project == $proj->id_project ? 'selected' : '' }}>{{ $proj->kode_project }} - {{ $proj->nama_project }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="mb-3" id="div_akun_kas">
                            ...
                            <select class="form-select" name="akun_kas_bank">
                                ...
                                @foreach($akunKas as $a)
                                    <option value="{{ $a->kode_akun }}" {{ $pembelian->akun_kas_bank == $a->kode_akun ? 'selected' : '' }}>{{ $a->nama_akun }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Keterangan</label>
                            <textarea class="form-control" name="keterangan" rows="2">{{ $pembelian->keterangan }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card bg-light mb-3">
                    <div class="card-body text-center">
                        <h3>Total Faktur</h3>
                        <h1 class="display-4 fw-bold text-primary" id="display_total">Rp {{ number_format($pembelian->total, 0, ',', '.') }}</h1>
                    </div>
                </div>
            </div>
        </div>
    ...
    function tambahBaris(data = null) {
        ...
        const html = `
            <tr id="row_${idx}">
                ...
                <td>
                    <input type="number" class="form-control form-control-sm" name="details[${idx}][harga_beli]" id="harga_${idx}" value="${data ? data.harga : ''}" onkeyup="hitungSubtotal(${idx})" onchange="hitungSubtotal(${idx})">
                </td>
                <td>
        ...
        if (e.key === 'Enter') {
            ...
            if (code) {
                ...
                if (b) {
                    ...
                    document.getElementById(`harga_${targetIdx}`).value = b.harga_beli || 0;
