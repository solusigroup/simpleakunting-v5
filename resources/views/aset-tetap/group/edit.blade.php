...
@section('title', 'Edit Kelompok Aset Tetap - Simple Akunting')

@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Kelompok Aset Tetap</h1>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('aset-tetap-group.update', $group->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="nama_kelompok" class="form-label">Nama Kelompok</label>
                            <input type="text" class="form-control @error('nama_kelompok') is-invalid @enderror" id="nama_kelompok" name="nama_kelompok" value="{{ old('nama_kelompok', $group->nama_kelompok) }}" required>
                            @error('nama_kelompok')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('umur_ekonomis') is-invalid @enderror" id="umur_ekonomis" name="umur_ekonomis" value="{{ old('umur_ekonomis', $group->umur_ekonomis) }}" required min="1">
                                @error('umur_ekonomis')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('metode_penyusutan') is-invalid @enderror" id="metode_penyusutan" name="metode_penyusutan" required>
                                    <option value="Garis Lurus" {{ old('metode_penyusutan', $group->metode_penyusutan) == 'Garis Lurus' ? 'selected' : '' }}>Garis Lurus (Straight Line)</option>
                                    <option value="Saldo Menurun" {{ old('metode_penyusutan', $group->metode_penyusutan) == 'Saldo Menurun' ? 'selected' : '' }}>Saldo Menurun (Declining Balance)</option>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('akun_aset') is-invalid @enderror" id="akun_aset" name="akun_aset">
                                ...
                                @foreach($akuns as $akun)
                                    <option value="{{ $akun->kode_akun }}" {{ old('akun_aset', $group->akun_aset) == $akun->kode_akun ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('akun_akumulasi_penyusutan') is-invalid @enderror" id="akun_akumulasi_penyusutan" name="akun_akumulasi_penyusutan">
                                ...
                                @foreach($akuns as $akun)
                                    <option value="{{ $akun->kode_akun }}" {{ old('akun_akumulasi_penyusutan', $group->akun_akumulasi_penyusutan) == $akun->kode_akun ? 'selected' : '' }}>
                                        {{ $akun->kode_akun }} - {{ $akun->nama_akun }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select select2 @error('akun_beban_penyusutan') is-invalid @enderror" id="akun_beban_penyusutan" name="akun_beban_penyusutan">
                                ...
                                @foreach($akuns as $akun)
                                    <option value="{{ $akun->kode_akun }}" {{ old('akun_beban_penyusutan', $group->akun_beban_penyusutan) == $akun->kode_akun ? 'selected' : '' }}>
                                        {{ $akun->kode_akun }} - {{ $akun->nama_akun }}
                                    </option>
                                @endforeach
                            </select>
