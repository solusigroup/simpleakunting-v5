...
@section('title', 'Edit Penawaran Penjualan - Simple Akunting')

@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Penawaran Penjualan #{{ $penawaran->no_penawaran }}</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{ route('penawaran.show', $penawaran->id_penawaran) }}" class="btn btn-sm btn-secondary">
                Batal
            </a>
        </div>
    </div>
    ...
    <form action="{{ route('penawaran.update', $penawaran->id_penawaran) }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="card mb-3">
                    <div class="card-header">Info Penawaran</div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">No Penawaran</label>
                                <input type="text" class="form-control" name="no_penawaran" value="{{ $penawaran->no_penawaran }}" readonly>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Tanggal</label>
                                <input type="date" class="form-control" name="tanggal_penawaran" value="{{ $penawaran->tanggal_penawaran }}" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="form-select" name="id_pelanggan" required>
                                    ...
                                    @foreach($pelanggan as $p)
                                        <option value="{{ $p->id_pelanggan }}" {{ $penawaran->id_pelanggan == $p->id_pelanggan ? 'selected' : '' }}>{{ $p->nama_pelanggan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="id_cabang" name="id_cabang" required>
                                    ...
                                    @foreach($cabang as $c)
                                        <option value="{{ $c->id }}" {{ $penawaran->id_cabang == $c->id ? 'selected' : '' }}>{{ $c->kode_cabang }} - {{ $c->nama_cabang }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="id_unit_usaha" name="id_unit_usaha" required>
                                    ...
                                    @foreach($unitUsaha as $u)
                                        <option value="{{ $u->id }}" data-cabang="{{ $u->id_cabang }}" {{ $penawaran->id_unit_usaha == $u->id ? 'selected' : '' }}>{{ $u->kode_unit }} - {{ $u->nama_unit }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="form-select" id="id_project" name="id_project">
                                    ...
                                    @foreach($projects as $proj)
                                        <option value="{{ $proj->id_project }}" data-unit="{{ $proj->id_unit_usaha }}" {{ $penawaran->id_project == $proj->id_project ? 'selected' : '' }}>{{ $proj->kode_project }} - {{ $proj->nama_project }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="status" required>
                                ...
                                <option value="Dikirim" {{ $penawaran->status == 'Dikirim' ? 'selected' : '' }}>Dikirim</option>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="keterangan" rows="2">{{ $penawaran->keterangan }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card bg-light mb-3">
                    <div class="card-body text-center d-flex flex-column justify-content-center" style="min-height: 200px;">
                        ...
                        <h1 class="display-5 fw-bold text-primary" id="display_total">Rp {{ number_format($penawaran->total, 0, ',', '.') }}</h1>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <table class="table table-bordered mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th width="15%">Harga Penawaran</th>
                                <th width="10%">Qty</th>
...
<script>
    ...
    const existingDetails = @json($penawaran->details);
    ...
    function tambahBaris(data = null) {
            ...
            return `<div class="searchable-select-option" 
                ...
                <div class="d-flex justify-content-between align-items-center w-100" style="gap: 15px;">
                    ...
                    <div class="text-end" style="font-size: 0.72rem; line-height: 1.35; flex-shrink: 0; min-width: 140px; border-left: 1px solid #f0f0f0; padding-left: 10px;">
                        ...
                        <div class="text-muted" style="font-size: 0.68rem; margin-top: 1px;">Jual: Rp ${jual}</div>
                    </div>
                </div>
            </div>`;
        }).join('');
        ...
        const html = `
            <tr id="row_${idx}">
                <td>
                    <div class="searchable-select" id="ss_${idx}">
                        ...
                        <div class="searchable-select-trigger" id="ss_trigger_${idx}">
                            <span class="trigger-text ${data ? '' : 'placeholder'}">${data ? (data.barang ? `${data.barang.kode_barang} - ${data.barang.nama_barang}` : '🔍 Cari atau pilih barang...') : '🔍 Cari atau pilih barang...'}</span>
                ...
                <td>
                    <input type="number" class="form-control form-control-sm" name="details[${idx}][harga]" id="harga_${idx}" value="${data ? data.harga : ''}" onkeyup="hitungSubtotal(${idx})" onchange="hitungSubtotal(${idx})" required>
                </td>
                <td>
